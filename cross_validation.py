...
def cross_validation_sgd(y, x, k_indices, k, initial_w, max_iters, gamma, degree):
    ...
    if initial_w == 'ones':
        ...
        np.random.seed(42)
        ini_w = np.random.rand(xpoly_tr.shape[1], 1)
    w, loss_tr = mean_squared_error_sgd(y_tr, xpoly_tr,ini_w,max_iters,gamma, divergence_ratio=1.01)
    # calculate the loss for test data:
    loss_tr = np.sqrt(2 * compute_loss(y_tr, xpoly_tr, w, "mse"))
    loss_te = np.sqrt(2 * compute_loss(y_te, xpoly_te, w,"mse"))
    return loss_tr, loss_te, w

def best_selection_sgd(y, x, degrees, k_fold, initial_ws_shape, max_iters,gammas, seed = 1):
    # split data in k fold
    k_indices = build_k_indices(y, k_fold, seed)
            ...
            for l in lambdas:
                ...
                for initial_w in initial_ws_shape:
                    ...
                    for k in range(k_fold):
                        ...
                        _, loss_te,_ = cross_validation_reg_logistic(y,x,k_indices, k,initial_w, l,max_iters, gamma, degree)
